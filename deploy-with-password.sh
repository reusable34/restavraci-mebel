...
# Скрипт деплоя с поддержкой пароля (менее безопасно, но работает)
# Использование: ./deploy-with-password.sh
...
echo "⚠️  Будет запрошен пароль для подключения"
...
echo "📤 Копирую файлы на сервер (потребуется пароль)..."
scp -P "$REMOTE_PORT" "${TEMP_DIR}/app.tar.gz" ${REMOTE_USER}@${REMOTE_HOST}:/tmp/

echo "🔧 Устанавливаю зависимости и настраиваю на сервере (потребуется пароль)..."
...
echo ""
echo "📝 Следующие шаги:"
echo "   1. Подключитесь к серверу: ssh root@${REMOTE_HOST}"
...
echo ""
echo "🔍 Проверка логов: ssh root@${REMOTE_HOST} 'journalctl -u ${SERVICE_NAME} -f'"
